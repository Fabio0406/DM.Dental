...
class Lote {
  ...
  static async getLotesFIFO(idInsumo) {
    try {
      const result = await pool.query(
        `SELECT l.*, i.nombre_generico, i.codigo, CASE WHEN l.fecha_vencimiento < CURRENT_DATE THEN TRUE ELSE FALSE END AS esta_vencido, (l.fecha_vencimiento - CURRENT_DATE) AS dias_hasta_vencimiento FROM lotes l INNER JOIN insumos i ON l.id_insumo = i.id_insumo WHERE l.id_insumo = $1 AND l.aplicaciones_disponibles > 0 AND l.fecha_vencimiento >= CURRENT_DATE ORDER BY l.fecha_vencimiento ASC, l.id_lote ASC`,
        [idInsumo]
      );
      return result.rows;
    } catch (error) {
  ...
  static async getLotesVencidosNoNotificados(idInsumo = null) {
    try {
      let query = `SELECT l.*, i.nombre_generico, i.codigo, i.presentacion, (CURRENT_DATE - l.fecha_vencimiento) AS dias_vencido FROM lotes l INNER JOIN insumos i ON l.id_insumo = i.id_insumo WHERE l.fecha_vencimiento < CURRENT_DATE AND l.aplicaciones_disponibles > 0 AND l.vencimiento_notificado = FALSE`;
  ...
  static async marcarVencimientoNotificado(idLotes) {
    ...
    try {
      ...
      await client.query(
        `UPDATE lotes SET vencimiento_notificado = TRUE WHERE id_lote = ANY($1::int[])`,
        [lotes]
      );
  ...
  static async getLotesByInsumo(idInsumo) {
    try {
      const result = await pool.query(
        `SELECT l.*, CASE WHEN l.fecha_vencimiento < CURRENT_DATE THEN 'VENCIDO' WHEN l.aplicaciones_disponibles = 0 THEN 'AGOTADO' ELSE 'ACTIVO' END AS estado_lote, (l.fecha_vencimiento - CURRENT_DATE) AS dias_hasta_vencimiento FROM lotes l WHERE l.id_insumo = $1 ORDER BY CASE WHEN l.aplicaciones_disponibles > 0 THEN 0 ELSE 1 END, l.fecha_vencimiento ASC`,
        [idInsumo]
      );
      return result.rows;
    } catch (error) {
  ...
  static async getLoteById(idLote) {
    try {
      const result = await pool.query(
        `SELECT l.*, i.nombre_generico, i.codigo, i.presentacion, u.nombres || ' ' || u.apellidos AS registrado_por, CASE WHEN l.fecha_vencimiento < CURRENT_DATE THEN 'VENCIDO' WHEN l.aplicaciones_disponibles = 0 THEN 'AGOTADO' ELSE 'ACTIVO' END AS estado_lote FROM lotes l INNER JOIN insumos i ON l.id_insumo = i.id_insumo LEFT JOIN usuarios u ON l.id_usuario = u.ci WHERE l.id_lote = $1`,
        [idLote]
      );
  ...
  static async actualizarAplicaciones(idLote, aplicacionesNuevas) {
    try {
      const result = await pool.query(
        `UPDATE lotes SET aplicaciones_disponibles = $1 WHERE id_lote = $2 RETURNING *`,
  ...
  static async crearLote(loteData) {
    ...
    try {
      const result = await pool.query(
        `INSERT INTO lotes (id_insumo, id_formulario, numero_lote, fecha_vencimiento, cantidad_insumos, aplicaciones_disponibles, costo_total, id_usuario) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
        [
          id_insumo,
          id_formulario || null,
          numero_lote,
...
export default Lote; // ⬅️ CAMBIADO: module.exports a export default