...
class Consumo {
        ...
        try {
            ...
            for (const insumo of insumos) {
                ...
                const lotesResult = await client.query(
                    `SELECT * FROM lotes WHERE id_insumo = $1 AND aplicaciones_disponibles > 0 AND fecha_vencimiento >= CURRENT_DATE ORDER BY fecha_vencimiento ASC, id_lote ASC`,
                    [id_insumo]
                );
                ...
                for (const lote of lotesResult.rows) {
                    ...
                    const ultimoMovimiento = await client.query(
                        `SELECT saldo_valorado, costo_unitario FROM detalle_kardex WHERE id_kardex = $1 ORDER BY fecha DESC, id_detalle DESC LIMIT 1`,
                        [idKardex]
                    );
                    ...
                    const movimientoResult = await client.query(
                        `INSERT INTO detalle_kardex (id_kardex, id_lote, fecha, entradas, salidas, ajustes, saldo, clave_doc, recibido_de, recepcionado_por, costo_unitario, saldo_valorado) VALUES ($1, $2, CURRENT_TIMESTAMP, 0, $3, 0, $4, $5, $6, $7, $8, $9) RETURNING id_detalle`,
                        [
                            idKardex,
                            lote.id_lote,
    ...
    static async registrarAjuste(ajusteData) {
        ...
        try {
            ...
            const ultimoMovimiento = await client.query(
                `SELECT saldo, saldo_valorado, costo_unitario FROM detalle_kardex WHERE id_kardex = $1 ORDER BY fecha DESC, id_detalle DESC LIMIT 1`,
                [idKardex]
            );
            ...
            await client.query(
                `INSERT INTO detalle_kardex (id_kardex, id_lote, fecha, entradas, salidas, ajustes, saldo, clave_doc, recibido_de, recepcionado_por, costo_unitario, saldo_valorado) VALUES ($1, $2, CURRENT_TIMESTAMP, 0, 0, $3, $4, $5, $6, $7, $8, $9)`,
                [
                    idKardex,
                    id_lote,
    ...
    static async getDetallesConsumo(idDetalle) {
        try {
            const result = await pool.query(
                `SELECT dk.id_detalle, dk.fecha, dk.salidas AS cantidad, dk.clave_doc AS numero_recibo, dk.saldo, dk.saldo_valorado, dk.costo_unitario, i.id_insumo, i.codigo, i.nombre_generico, i.presentacion, l.numero_lote, l.fecha_vencimiento, k.numero_kardex, k.gestion, u.nombres || ' ' || u.apellidos AS usuario FROM detalle_kardex dk INNER JOIN kardex k ON dk.id_kardex = k.id_kardex INNER JOIN insumos i ON k.id_insumo = i.id_insumo LEFT JOIN lotes l ON dk.id_lote = l.id_lote LEFT JOIN usuarios u ON dk.recepcionado_por = u.ci WHERE dk.id_detalle = $1`,
                [idDetalle]
            );
    ...
    static async findByUserAndDate(userId, fechaInicio = null, fechaFin = null) {
        try {
            let query = `SELECT dk.id_detalle, dk.fecha, dk.salidas AS cantidad, dk.clave_doc AS numero_recibo, dk.saldo_valorado, i.codigo, i.nombre_generico, l.numero_lote FROM detalle_kardex dk INNER JOIN kardex k ON dk.id_kardex = k.id_kardex INNER JOIN insumos i ON k.id_insumo = i.id_insumo LEFT JOIN lotes l ON dk.id_lote = l.id_lote WHERE dk.recepcionado_por = $1 AND dk.salidas > 0`;
    ...
    static async getStats(userId, fechaInicio, fechaFin) {
        try {
            const result = await pool.query(
                `SELECT COUNT(DISTINCT dk.clave_doc) AS total_servicios, SUM(dk.salidas) AS total_aplicaciones_consumidas, COUNT(DISTINCT k.id_insumo) AS insumos_diferentes_usados, SUM(CASE WHEN dk.salidas > 0 THEN (SELECT costo_unitario FROM detalle_kardex WHERE id_kardex = dk.id_kardex AND fecha <= dk.fecha ORDER BY fecha DESC LIMIT 1) * dk.salidas ELSE 0 END) AS valor_total_consumido FROM detalle_kardex dk INNER JOIN kardex k ON dk.id_kardex = k.id_kardex INNER JOIN insumos i ON k.id_insumo = i.id_insumo WHERE dk.recepcionado_por = $1 AND dk.salidas > 0 AND dk.fecha >= $2 AND dk.fecha <= $3`,
