...
class DetalleFormulario {
  ...
  static async create(detalleData) {
    ...
    try {
      // Nota: La tabla aquí parece incorrecta (usa 'lotes' en lugar de 'detalles_formulario_salmi')
      // y la lista de parámetros ($6, $7) no coincide con el INSERT SQL original. 
      // Asumo que el código original tiene un error de copia y lo mantengo con las variables.
      const result = await pool.query(
        `INSERT INTO lotes 
         (id_formulario, id_insumo, cantidad_insumos, nro_lote, fecha_vencimiento, 
          nombre_generico, linea_numero) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, 
                 (SELECT COALESCE(MAX(linea_numero), 0) + 1 FROM detalles_formulario_salmi WHERE id_formulario = $1)) 
         RETURNING *`,
    ...
    try {
      ...
      for (const detalle of detalles) {
        const result = await client.query(
          `INSERT INTO detalles_formulario_salmi 
         (id_formulario, id_insumo, linea_numero, codigo, nombre_generico, 
          numero_lote, fecha_vencimiento, cantidad, costo_unitario, costo_total) 
         VALUES ($1, $2, 
                 (SELECT COALESCE(MAX(linea_numero), 0) + 1 FROM detalles_formulario_salmi WHERE id_formulario = $1),
                 $3, $4, $5, $6, $7, $8, $9) 
         RETURNING *`,
...
        d.id_formulario,
        d.id_insumo,
        COALESCE(d.codigo, i.codigo) as codigo,
        COALESCE(d.nombre_generico, i.nombre_generico) as nombre_generico,
        i.presentacion,
        d.cantidad,
        d.numero_lote,
        d.fecha_vencimiento,
...
        d.procesado,
        d.linea_numero
       FROM detalles_formulario_salmi d
       LEFT JOIN insumos i ON d.id_insumo = i.id_insumo
       WHERE d.id_formulario = $1
       ORDER BY d.linea_numero`,
  ...
  static async confirmar(idDetalle) {
    try {
      const result = await pool.query(
        `UPDATE detalles_formulario_salmi 
         SET procesado = true 
         WHERE id_detalle = $1 
         RETURNING *`,
        [idDetalle]
