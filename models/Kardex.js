...
class Kardex {
  ...
  static async obtenerKardexActual(idInsumo, fechaInicio = null, fechaFin = null) {
    try {
      // Obtener info del Kardex activo con forma_farmaceutica y concentracion
      ...
      // Construir query de movimientos con filtros opcionales de fecha
      ...
      const params = [kardex.id_kardex];

      if (fechaInicio) {
        params.push(fechaInicio);
        movimientosQuery += ` AND dk.fecha >= $${params.length}`;
      }
      ...
      movimientosQuery += ` ORDER BY dk.fecha ASC, dk.id_detalle ASC`;

      const movimientos = await pool.query(movimientosQuery, params);
